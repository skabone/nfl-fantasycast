(() => {
  ...
  function summaries(leagues,{now=Date.now()}={}){
    ...
    const rows=leagues.map(league=>{
      const base={leagueId:league.id,percent:null,kind:'unavailable',label:'Chance not captured',meta:'No win chance captured',detail:'No verified platform win percentage is saved for this team.',attentionRank:null,sourceUrl:null,stale:false};
      ...
      if(prior||wrongTeam)return {...base,label:prior?'Prior-week chance':'Different team capture',meta:`Win chance captured ${full}`,detail:`The saved percentage is for ${row.season}, week ${row.week}. A matching current capture is needed.`,sourceUrl:row.sourceUrl,stale:true};
      if(!Number.isFinite(row.ownPercent))return {...base,meta:`No win chance shown at the ${short} capture · ${row.provider}`,detail:'The platform did not display a verified win percentage at this check.',sourceUrl:row.sourceUrl};
