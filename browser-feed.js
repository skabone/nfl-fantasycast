(() => {
  ...
  async function cached(key,label,url,loader,ttl=25000,refresh=false){
  ...
  function detail(raw,id){
    ...
    const ordered=[...plays.values()].sort((a,b)=>(b.sequence||0)-(a.sequence||0));if(ordered.length){g.lastPlay=ordered[0].text;if(g.state==='in'){g.fieldPosition=field(ordered[0].end,g.home,g.away);g.possessionTeamId=g.fieldPosition?.possessionTeamId||null;g.downDistance=ordered[0].end.downDistanceText||null;}}
  ...
  async function getDashboard(date=today(),leagueId=null,refresh=false){
    ...
    if(leagueId&&!configured.some(l=>String(l.id)===String(leagueId))&&leagueId!==espnId)throw Error('Import a profile containing this league first.');
    const url=ESPN+'scoreboard?dates='+date.replace(/-/g,''),[[b,bs],[state,ss]]=await Promise.all([cached('board:'+date,'ESPN NFL scoreboard',url,async()=>board(await fetchJSON(url)),25000,refresh),cached('state:nfl','Sleeper NFL week',SLEEPER+'state/nfl',()=>fetchJSON(SLEEPER+'state/nfl'),3600000)]);
    ...
    if(leagueId&&leagueId===espnId){fantasy=snapshot(es);fs=source('ESPN fantasy capture',es.sourceUrls?.matchup,'snapshot',es.observedAt);notices.push('ESPN fantasy scores are a dated imported capture; they do not auto-refresh. NFL games refresh independently.');}
    else if(leagueId&&week&&p?.sleeperUserId){const url=SLEEPER+'league/'+leagueId+'/matchups/'+week;[fantasy,fs]=await cached('matchup:'+p.sleeperUserId+':'+leagueId+':'+season+':'+week,'Sleeper weekly matchup',url,()=>liveMatchup(String(leagueId),week,p),25000,refresh);}
