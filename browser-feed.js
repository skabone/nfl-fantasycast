(() => {
  ...
  // ESPN's headshot when the provider gave us an ESPN id, otherwise Sleeper's own. Sleeper's directory
  // leaves espn_id empty for most players — on real rosters that was 29 of 40 starters with a blank avatar.
  function headshot(sleeperId,espnId,position,team){
    if(['DEF','DST','D/ST'].includes(position)){const club=String(team||sleeperId||'').toLowerCase();return /^[a-z]{2,3}$/.test(club)?safeURL('https://sleepercdn.com/images/team_logos/nfl/'+club+'.png',true):null;}
    if(espnId&&/^\d+$/.test(String(espnId)))return safeURL('https://a.espncdn.com/i/headshots/nfl/players/full/'+espnId+'.png',true);
    if(sleeperId&&/^\d+$/.test(String(sleeperId)))return safeURL('https://sleepercdn.com/content/nfl/players/'+sleeperId+'.jpg',true);
    return null;
  }
